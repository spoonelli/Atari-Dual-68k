...
module jt51_acc(
    ...
    input           [2:0]   gain_I,   // MIX-100: this channel's user gain
...
reg signed [13:0] op_raw;

always @(*) begin
    if( ne && op31_acc ) // cambiar a OP 31
        op_raw = { {2{noise_mix[11]}}, noise_mix };
    else
        op_raw = op_out;
end

// MIX-100 per-channel user gain. gain_I is the gain of the channel whose
// parameters are on the pipeline this cycle (see jt51.v: it is selected by
// `cycles`, which is jt51_reg's own `cur` counter - the same index that
// picks reg_rl[ch] for rl_I - so it is aligned by construction, not by
// tuning). 7 = unity, 0 = mute; scale is (g+1)/8 on the operator value
// before accumulation, which scales the whole channel regardless of
// algorithm since every contributing operator is scaled equally.
wire signed [17:0] op_scaled = op_raw * $signed({1'b0, gain_I} + 4'd1);
wire signed [13:0] op_val    = (gain_I == 3'd7) ? op_raw : op_scaled[16:3];
