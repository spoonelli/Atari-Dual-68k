...
--            rtl/lib/TG68K/TG68K.vhd
--            submodule commit 4fc5ed8f661b1af0526c7aa3bff6dc92cbad2353
-- Vendored : 2026-08-23.  Copied verbatim, then ONE addition: the `LOCK`
--            output port, fed straight from the kernel's new `lock_rmw`
...
-- sync interface
	...
	VPA           : in  std_logic;
	VMA           : out std_logic;
-- TASLOCK-102 (local addition): active-high read-modify-write indivisibility
-- window.  High from the clock that launches the READ half of a memory RMW
-- (TAS, and every other memory-destination ALU op) to the clock that retires
-- its WRITE-BACK, /AS gaps included.  The real 68000 signals this by holding
-- /AS low across both halves; this core cannot, so it exports the window and
-- the surrounding fabric does the serialising.  See the file header.
	LOCK          : out std_logic
);
end TG68K;
...
port(
	...
	lock_rmw       : out std_logic     -- TASLOCK-102
