...
| C6 | **Special (MPR2) sprite pixels of pen 1 are drawn as shadows** | **GAL 136069.100V, decoded from the romset fuse map** (`sim/tools/gal16v8.py`; MAME's `jedutil` says the same, we just could not run it): the line-buffer write PAL writes normal pixels when pen != 0 and MAT6 = 0, pulses `/STON` / `/STOFF` for special pens with bit 1 / bit 2, and has ONE extra write term `MSD0 & /MSD1 & /MSD2 & /MSD3 & MAT6` — a special pixel whose pen is exactly 1 IS written. GAL 100T then sees MPX = 1: M7 (playfield wins) and, for MO colour != 0, SHADE (colour-RAM +0x100). The game relies on it: the FACTORY MAP marks travelled routes and the START box with 2x2 *pen-1* specials in colour 4 (ROM frame table 0x6A938; pen-1 twins of the pen-6 stain markers at tiles 22A2../22C4..), and populates the +0x100 bank with I=4 same-hue copies. **MAME's `screen_update_eprom` `continue`s on `mopriority & 4` BEFORE SHADE**, so MAME never dims those routes (its level-1 and level-3 maps stay bright); this core had transcribed that (`hit = occ && !special`). The owner's PCB capture (t406–t410, frame 24402) shows the routes at exactly the +0x100 entries. Fixed MOSHADE-162: `escape_mob.v` `hit = occ && (!special \|\| pen == 1)`. Record: [`investigations/MAP_HALFTONE.md`](investigations/MAP_HALFTONE.md). |
| C7 | **M7 does not enter the playfield colour-RAM address** | MAME ORs `0x080` into the playfield pen on M7 (its reading of "M7 is passed as the upper MO bit to the GPC"), and `escape_prio.v` transcribed it as `pf_color[3] \| m7`. The game's colour RAM contradicts it: on the map and in gameplay it populates the SHADE bank for colour 0 only (`0x300–0x30F`) and **never** `0x380–0x38F`, the bank SHADE\|M7 would select — so every shadowed playfield pixel indexed an empty bank and came out black (the "erased" travelled routes, the 1-px slivers being the un-shadowed edge). Shadowed playfield indexes `pf \| 0x100` only; M7's playfield-side effect is the layer decision. The PCB's dim START box measures as `0x30E` (`4FF0`). Fixed MOSHADE-162 in RTL and in `sim/tools/mo_priority_model.py`; `tb_prio` exhaustive equivalence re-proven (507,904/507,904). Reported-upstream status: not yet. |
...
| Subsystem | Result |
|---|---|
| MO/PF priority comparator | **507,904 / 507,904 = 100.0000%** against the GAL 100T equations (MAME's transcription, minus the `0x080` M7 term — see C7) |
