...
# the playfield fetches the arbiter actually serves.  See sim/tb/tb_mister_moarb.v.
#
...
docker run --rm -v "$REPO":/work -w /work "$IMG" bash -c "
  iverilog -g2012 -o sim/build/tb_mister_moarb.vvp \
    ...
    sim/tb/tb_mister_moarb.v && \
  vvp sim/build/tb_mister_moarb.vvp" | tee sim/build/tb_mister_moarb.log

grep -q '^RESULT: PASS' sim/build/tb_mister_moarb.log
