...
for XS in ${XSCROLLS:-0 2 3 5 6}; do
  docker run --rm -v "$REPO":/work -w /work "$IMAGE" bash -c "
    iverilog -g2012 -Ptb_pfline.XSCROLL=$XS -Ptb_pfline.GFX_LAT=${GFX_LAT:-6} -Ptb_pfline.GFX_LAT=${GFX_LAT:-6} -Ptb_pfline.LEAD=${LEAD:-16} -Ptb_pfline.NCH=${NCH:-2} -Ptb_pfline.RP_OFF=${RPO:-0} -Ptb_pfline.PFC_EN=${PFC:-0} -Ptb_pfline.MAPMOD=${MAPMOD:-64} -Ptb_pfline.GFX_LAT=${GFX_LAT:-6} -o sim/build/tb_pfline.vvp \
